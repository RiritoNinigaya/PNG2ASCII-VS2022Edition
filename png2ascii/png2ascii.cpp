                            ...
                            unsigned* input_height)
{
    ...
    float brightness;
    int x, index;
    char *art_chars = (char*)"@%#*+=-:. "; /* ASCII char range, darkest to lightest */
    
    /* Sets image as buffer with pattern RGBARGBARGBA */
    error = lodepng_decode32_file((unsigned char**)&image, (unsigned int*)&width, (unsigned int*)&height, filename);
...
int main(int argc, const char *argv[]) {
    ...
    int width, height;
    unsigned char *output = png_to_ascii(argv[1], (unsigned int*)&width, (unsigned int*)&height);
